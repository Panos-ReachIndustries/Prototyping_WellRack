"""
visualize_with_gridtracker12_imgs.py
====================================
Image batch visualization with GridTracker v12 and evaluation metrics.
...
Runs the same logic as visualize_with_gridtracker12_video.py but processes images
...
Uses v12 improvements:
- KeypointVialsDetector for robust vial detection
...
from visualize_with_gridtracker12_video import (
...
def process_images_from_folder(
    ...
    """
    Process images from a folder with GridTracker v12 overlay and evaluation.

    Args:
        input_folder: Path to folder containing images
        output_folder: Optional output folder path (default: input_folder/folder_GridTracker12_results)
        ...
        checkpoint_path: Path to keypoint model checkpoint
        keypoint_threshold: Keypoint detection threshold
        keypoint_nms_radius: NMS radius for keypoint detection
        enable_lid_classification: Enable lid state classification
        lid_classifier_*: LidStateClassifier parameters (v12 defaults)
    """
    ...
    if output_folder is None:
        output_path = input_path / "folder_GridTracker12_results"
    else:
    ...
    for idx, image_file in enumerate(image_files):
        ...
        output_file = output_path / f"{image_file.stem}_gridtracker12.png"
        ...
        if show:
            cv2.imshow("WellRack GridTracker v12 Overlay", annotated_frame)
    ...
    if len(all_vials_times) > 0:
        ...
        logger.info("=" * 60)
        logger.info("PROFILING SUMMARY - GridTracker v12 (All Images)")
        logger.info("=" * 60)
        logger.info(f"Total images processed: {len(all_vials_times)}")
        logger.info("")
        logger.info(f"Vials Detection (Keypoint Model):")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize images with WellRack GridTracker v12 (KeypointVialsDetector) and run evaluation."
    )
    parser.add_argument(
        "--image-folder",
        type=str,
