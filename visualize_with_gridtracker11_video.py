"""
visualize_with_gridtracker11_video.py
======================================
Video visualisation with the v11 improved LidStateClassifier.

Improvements over v10 (derived from ToDo.md engineering analysis):

Fix B - FFT texture features
    Replaces raw mean-absolute-difference (MAD) between strips with a
    normalised mid-high-frequency power ratio computed in the frequency domain.
    Open wells expose a periodic ring structure (high FFT power); closed lids
    are flat (low FFT power).  Illumination-invariant and specular-reflection
    resistant.

Fix C - Pairwise K-Means clustering (no D1 assumption)
    All columns are compared via their FFT texture feature.  K-Means (k=2)
    partitions them into OPEN / CLOSED clusters without requiring D1 to be a
    reliable reference.  Falls back to the v10 reference-subtraction gate when
    all columns look visually similar (feature range < min_feature_separation).

Fix D - Per-frame lid classification
    Lid classification is decoupled from the grid-update interval and runs on
    every frame using the last-known stable line positions.  This increases
    effective temporal buffer density 5x over v10.

Also uses better-tuned baseline params: vial_count_threshold=0.20,
diff_threshold_method="median_iqr", iqr_multiplier=0.8, min_visual_threshold=40.0.
"""
import argparse
import json
import logging
...
class GridTrackerWithExternalMask:
    ...
    def __init__(
        ...
        lid_classifier_vial_count_threshold: float = 0.20,   # v11: tightened from 0.5
        lid_classifier_reference_state: str = "CLOSED_LID",
        lid_classifier_diff_threshold_method: str = "median_iqr",  # v11: improved method
        lid_classifier_iqr_multiplier: float = 0.8,          # v11: tuned
        lid_classifier_min_visual_threshold: float = 40.0,   # v11: tuned
        lid_classifier_use_fft_features: bool = True,        # Fix B
        lid_classifier_use_pairwise_clustering: bool = True,  # Fix C
        lid_classifier_min_feature_separation: float = 0.08,  # v11: raised from 0.05 (gap check)
        lid_classifier_profile_sigma_factor: float = 0.20,   # Fix E: Gaussian weighting suppresses rack cell-wall contamination
    ):
        self.dominant_lines = []
        ...
        if self.enable_lid_classification:
            self.lid_classifier = LidStateClassifier(
                ...
                use_fft_features=lid_classifier_use_fft_features,        # Fix B
                use_pairwise_clustering=lid_classifier_use_pairwise_clustering,  # Fix C
                min_feature_separation=lid_classifier_min_feature_separation,
                profile_sigma_factor=lid_classifier_profile_sigma_factor,  # Fix E
                debug=False,
            )
            logger.info(
                f"LidStateClassifier v11 initialized: "
    ...
    def track_frame_with_mask(self, frame: np.ndarray, plate_mask: np.ndarray | None):
        ...
        # Fix D: run lid classification every frame using last-known stable
        # line positions.  In v10 this was gated behind grid_was_updated, so
        # at grid_update_interval=5 it only ran every 5th frame and the 5-frame
        # majority-vote buffer contained only 1 real observation per 5 frames.
        # Running every frame gives the buffer 5x more data and responds ~5x
        # faster to genuine lid-state changes.
        if self.enable_lid_classification and self.lid_classifier is not None:
            if (self.current_grid_points is not None and
                ...
                try:
                    ...
                    logger.debug(
                        f"Frame {self.frame_count}: lid states "
                        f"{'(grid updated) ' if grid_was_updated else ''}"
                        f"{self.lid_states}"
                    )
...
def visualize_video(
    ...
    # Initialize GridTracker with v11 lid classification
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="output_gridtracker_overlay_11.mp4",
        help="Path to save annotated MP4 (default: output_gridtracker_overlay_new.mp4).",
    )
    parser.add_argument(
        "--plate-id",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.20,
        help="Vial count ratio threshold for structural gate (v11 default: 0.20).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-fft-features",
        action="store_true",
        help="Disable FFT texture features (Fix B); fall back to raw MAD.",
    )
    parser.add_argument(
        "--no-pairwise-clustering",
        action="store_true",
        help="Disable gap-based pairwise split (Fix C); use D1-reference subtraction.",
    )
    parser.add_argument(
        ...
        help=(
            "Gaussian weighting sigma for FFT profile (Fix E). "
            "0.0 = plain mean (box filter); 0.20 = recommended default. "
            "Suppresses rack cell-wall contamination in edge column strips."
        ),
    )
